...
$this->title	= 'All Files | ' . $coreProperties->getSiteTitle();
...
<div class="row header-content">
	<div class="col col12x8 header-actions">
		...
		<span class="wrap-sort">
			...
			<?= $dataProvider->sort->link( 'directory', [ 'class' => 'sort btn btn-medium' ] ) ?>
		</span>
	</div>
		...
		<table>
			<thead>
				<tr>
					...
					<th>Directory
						<span class='box-icon-sort'>
							<span sort-order='directory' class="icon-sort <?php if( strcmp( $sortOrder, 'directory' ) == 0 ) echo 'icon-up-active'; else echo 'icon-up';?>"></span>
							<span sort-order='-directory' class="icon-sort <?php if( strcmp( $sortOrder, '-directory' ) == 0 ) echo 'icon-down-active'; else echo 'icon-down';?>"></span>
						</span>
					</th>
					<th>Path
						<span class='box-icon-sort'>
							<span sort-order='url' class="icon-sort <?php if( strcmp( $sortOrder, 'url' ) == 0 ) echo 'icon-up-active'; else echo 'icon-up';?>"></span>
							<span sort-order='-url' class="icon-sort <?php if( strcmp( $sortOrder, '-url' ) == 0 ) echo 'icon-down-active'; else echo 'icon-down';?>"></span>
						</span>
					</th>
					<th>Created on
						<span class='box-icon-sort'>
			...
			<tbody>
				...
				foreach( $models as $file ) {
					...
					$id			= $file->id;
					$editUrl	= Html::a( $file->name, [ "update?id=$id" ] );
				?>
					<tr>
						<td><?= $editUrl ?></td>
						<td><?= $file->title ?></td>
						...
						<td>
							<span title="Download"><?= Html::a( "", $file->getFileUrl(), [ 'class' => 'cmti cmti-triangle-down', 'target' => '_blank' ] )  ?></span>
