...
class m160621_014408_core_data extends \cmsgears\core\common\base\Migration {
	...
	private function insertNotificationTemplates() {
		...
		$templates = [
			// Users
			[ $this->master->id, $this->master->id, 'New User', CoreGlobal::TPL_NOTIFY_USER_NEW, null, 'notification', 'Trigger Notification to Admin, when new user is registered.', true, 'twig', 0, null, false, null, DateUtil::getDateTime(), DateUtil::getDateTime(), 'User registered - <b>{{model.email}}</b>', 'A new user <b>{{model.name}}</b>, <b>{{model.email}}</b> has been registered.{% if config.link %} Please follow this <a href="{{config.link}}">link</a>.{% endif %}{% if config.adminLink %} Please follow this <a href="{{config.adminLink}}">link</a>.{% endif %}', '{"config":{"admin":"1","user":"0","direct":"0","adminEmail":"0","userEmail":"0","directEmail":"0"}}' ],
